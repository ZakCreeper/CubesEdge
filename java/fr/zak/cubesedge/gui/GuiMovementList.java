...
public class GuiMovementList extends GuiListExtended
{
    ...
    public GuiMovementList(GuiConfig config, Minecraft mc)
    {
        ...
        for (int k = 0; k < j; ++k)
        {
            ...
            int l = mc.fontRenderer.getStringWidth(movement.getClass().getAnnotation(Movement.class).value());
    ...
    public class MovementEntry implements GuiListExtended.IGuiListEntry
    {
        ...
        {
            this.movement = mov;
            this.movementName = mov.getClass().getAnnotation(Movement.class).value();
            this.btnDisable = new GuiButton(0, 0, 0, 100, 18, "");
        }

        public void drawEntry(int p_148279_1_, int p_148279_2_, int p_148279_3_, int p_148279_4_, int p_148279_5_, Tessellator p_148279_6_, int p_148279_7_, int p_148279_8_, boolean p_148279_9_)
        {
            GuiMovementList.this.mc.fontRenderer.drawString(this.movementName, p_148279_2_ + 105 - GuiMovementList.this.lenghtName, p_148279_3_ + p_148279_5_ / 2 - GuiMovementList.this.mc.fontRenderer.FONT_HEIGHT / 2, 16777215);
            ...
            this.btnDisable.drawButton(GuiMovementList.this.mc, p_148279_7_, p_148279_8_);
        }
        ...
        {
            if (this.btnDisable.mousePressed(GuiMovementList.this.mc, p_148278_2_, p_148278_3_))
            {
            	if(this.movement.isMovementDisabled()){
                	this.movement.enable();
                	Property prop = Util.cfg.get("movements", this.movement.getClass().getAnnotation(Movement.class).value(), true);
                	prop.set(true);
                	...
                	for(Method m : movement.getClass().getDeclaredMethods()){
    					if(m.isAnnotationPresent(SubscribeEvent.class)){
    						if(m.getParameterTypes()[0].getName().contains("cpw")){
    							FMLCommonHandler.instance().bus().register(movement);
    						}
    						else if(m.getParameterTypes()[0].getName().contains("minecraftforge")){
    							MinecraftForge.EVENT_BUS.register(movement);
    						}
    					}
    				}
                	for(Field f : movement.getClass().getDeclaredFields()){
    					if(f.getGenericType().toString().contains(KeyBinding.class.getName())){
                	...
                	for(Method m : movement.getClass().getDeclaredMethods()){
    					if(m.isAnnotationPresent(SubscribeEvent.class)){
    						if(m.getParameterTypes()[0].getName().contains("cpw")){
    							FMLCommonHandler.instance().bus().unregister(movement);
    						}
    						else if(m.getParameterTypes()[0].getName().contains("minecraftforge")){
                	...
                	for(Field f : movement.getClass().getDeclaredFields()){
    					if(f.getGenericType().toString().contains(KeyBinding.class.getName())){
    						...
    						try {
    							...
    							((HashSet)ObfuscationReflectionHelper.getPrivateValue(KeyBinding.class, (KeyBinding) f.get(movement), 2)).remove("Cube's Edge");
    						} catch (IllegalArgumentException e) {
    							e.printStackTrace();
    						} catch (IllegalAccessException e) {
    							e.printStackTrace();
