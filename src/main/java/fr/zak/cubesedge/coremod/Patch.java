...
public class Patch {
	...
	public static void renderRightHand(EntityPlayerSP p, float p2, float p3,ItemRenderer renderer,
			float tick){
		GlStateManager.pushMatrix();
		Minecraft mc = ObfuscationReflectionHelper.getPrivateValue(ItemRenderer.class, renderer, 2 /*mc var*/);
		RenderManager renderManager = ObfuscationReflectionHelper.getPrivateValue(ItemRenderer.class, renderer, 6 /*renderManager var*/);
		float f2 = -0.3F * MathHelper.sin(MathHelper.sqrt_float(p3) * (float)Math.PI);
		float f3 = 0.4F * MathHelper.sin(MathHelper.sqrt_float(p3) * (float)Math.PI * 2.0F);
		float f4 = -0.4F * MathHelper.sin(p3 * (float)Math.PI);
		GlStateManager.translate(f2, f3, f4);
		GlStateManager.translate(0.64000005F, -0.6F, -0.71999997F);
		GlStateManager.translate(0.0F, p2 * -0.6F, 0.0F);
		GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
		float f5 = MathHelper.sin(p3 * p3 * (float)Math.PI);
		float f6 = MathHelper.sin(MathHelper.sqrt_float(p3) * (float)Math.PI);
		...
		renderplayer.func_177138_b(mc.thePlayer);
		GlStateManager.popMatrix();
	}

	public static void renderLeftHand(float tick, ItemRenderer renderer, float f1){
		Minecraft mc = ObfuscationReflectionHelper.getPrivateValue(ItemRenderer.class, renderer, 2 /*mc var*/);
		if(!((EntityPlayerSP)mc.thePlayer).isInvisible()){
			GlStateManager.pushMatrix();
...
//			}
			...
			float f13 = 0.8F;
			GlStateManager.translate(0.8F * f13, -0.75F * f13 - (1.0F - f1) * 0.6F,-0.9F * f13);
			GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
			GlStateManager.enableRescaleNormal();
			mc.getTextureManager()
			.bindTexture(((EntityPlayerSP)mc.thePlayer).getLocationSkin());
			GlStateManager.translate(-1.0F, 3.6F, 3.5F);
			GlStateManager.rotate(120.0F, 0.0F, 0.0F, 1.0F);
			GlStateManager.rotate(200.0F, 1.0F, 0.0F, 0.0F);
			GlStateManager.rotate(-135.0F, 0.0F, 1.0F, 0.0F);
			...
			//		renderplayer.getPlayerModel().onGround = 0.0F;
			//		renderplayer.modelBipedMain.setRotationAngles(0.0F, 0.0F, 0.0F,
			//				0.0F, 0.0F, 0.0625F, Minecraft.getMinecraft().thePlayer);
			renderplayer.getPlayerModel().bipedLeftArm.render(0.0625F);
			GlStateManager.popMatrix();
		}
	}

	@SideOnly(Side.CLIENT)
	public static void renderItemInFirstPerson(EntityPlayerSP p, float p2, float p3,ItemRenderer renderer,
			float tick) {
		//		GlStateManager.pushMatrix();
		//		renderRightHand(p, p2, p3, tick, renderer);
		//		GlStateManager.popMatrix();
//		orientLeftHand(p, tick, renderer, p2);
