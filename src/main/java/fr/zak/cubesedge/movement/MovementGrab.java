...
public class MovementGrab extends Movement {
	...
	public void control(EntityPlayerCustom playerCustom, EntityPlayer player,
		...
		if (!playerCustom.isSneaking) {
			if (((((Util.isCube(WorldUtil.getBlock(player.worldObj, x, y, z - 1)) && heading == 2)
					|| (Util.isCube(WorldUtil.getBlock(player.worldObj, x, y,
							z + 1)) && heading == 0)
					|| (Util.isCube(WorldUtil.getBlock(player.worldObj, x - 1, y, z)) && heading == 1) || (Util
					.isCube(WorldUtil.getBlock(player.worldObj, x + 1, y, z)) && heading == 3))
					&& ((!Util
							.isCube(WorldUtil.getBlock(player.worldObj, x, y + 1, z - 1)) && heading == 2)
							|| (!Util.isCube(WorldUtil.getBlock(player.worldObj, x, y + 1,
									z + 1)) && heading == 0)
							|| (!Util.isCube(WorldUtil.getBlock(player.worldObj, x - 1,
									y + 1, z)) && heading == 1) || (!Util
							.isCube(WorldUtil.getBlock(player.worldObj, x + 1, y + 1, z)) && heading == 3)) && (!Util
						.isCube(WorldUtil.getBlock(player.worldObj, x, y - 2, z)))))
					&& WorldUtil.getBlock(player.worldObj, x, y, z) != Blocks.ladder
		...
		// if(side == Side.CLIENT){
		// // Util.channel.sendToServer(new
		// CPacketPlayerAction(playerCustom.isGrabbing ? 0 : 1));
		// }
		//
		// if(side == Side.SERVER){
		...
		if (!player.worldObj.isRemote) {
			if (playerCustom.isGrabbing && !player.capabilities.isCreativeMode) {
				...
				float f1 = potioneffect != null ? (float) (potioneffect
						.getAmplifier() + 1) : 0.0F;
				...
				if (i > 0) {
					...
					damageEntity(EntityLivingBase.class, player,
							DamageSource.fall, (float) i);
				}
